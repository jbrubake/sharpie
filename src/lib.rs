...
impl Ship {
    ...
    pub fn convert(p: String) -> Result<Ship, Box<dyn Error>> {
        ...
        ship.hull.units     = lines.next().unwrap().parse()?;
        for b in ship.batteries.iter_mut() { b.units = lines.next().unwrap().parse()?; }
        ship.torps[0].units = lines.next().unwrap().parse()?;
        ship.armor.units    = lines.next().unwrap().parse()?;
        ...
        ship.torps[1].units = lines.next().unwrap().parse()?;
        ship.mines.units    = lines.next().unwrap().parse()?;
        ship.asw[0].units   = lines.next().unwrap().parse()?;
        ship.asw[1].units   = lines.next().unwrap().parse()?;
        ...
        for b in ship.batteries.iter_mut() { b.groups[1].above  = lines.next().unwrap().parse()?; }
        // b.groups[1].on was previously loaded but, sure, why not load them again?
        for b in ship.batteries.iter_mut() { b.groups[1].on     = lines.next().unwrap().parse()?; }
        for b in ship.batteries.iter_mut() { b.groups[1].below  = lines.next().unwrap().parse()?; }
        ...
        for b in ship.batteries.iter_mut() {
            b.groups[0].on = b.mount_num -
                ...
                b.groups[1].above - b.groups[1].below;
        }
    ...
    pub fn report(&self) {
        ...
        if self.ship_type() != "" {
            println!("{}", self.ship_type());
        }

        if self.hull.cb() <= 0.0 || self.hull.cb() > 1.0
            { println!("DESIGN FAILURE: Displacement impossible with given dimensions"); }
        if self.hull.d() < (self.wgt_broad() / 4.0)
            { println!("DESIGN FAILURE: Gun weight too much for hull"); }
        if self.wgt_armor() > self.hull.d()
            { println!("DESIGN FAILURE: Armour weight too much for hull"); }
        if self.str_comp() < 0.5
            { println!("DESIGN FAILURE: Overall load weight too much for hull"); }
        if self.stability_adj() > 0.0
            { println!("DESIGN FAILURE: Ship will capsize"); }

        println!("");

        println!("Displacement:"); // {{{3
        println!("\t{:.0} t light; {:.0} t standard; {:.0} t normal; {:.0} t full load",
            (self.d_lite() as i64),
            (self.d_std() as i64),
            (self.hull.d() as i64),
            (self.d_max() as i64));
        ...
        println!("Dimensions: Length (overall / waterline) x beam x draught (normal/deep)"); // {{{3
        println!("\t({:.2} ft / {:.2} ft) x {:.2} ft x ({:.2} / {:.2} ft)",
            self.hull.loa(),
            self.hull.lwl(),
            self.hull.b,
            self.hull.t,
        ...
        for (i, b) in self.batteries.iter().enumerate() {
            ...
            if b.num == 0 { continue; }
            println!("\t{} - {:.2}\" {:.1} cal gun{} - {:.2}lbs shells, {:.0} per gun",
                b.num,
                b.cal,
                b.len,
                match b.num { 1 => "", _ => "s", },
            ...
            for (i, sb) in b.groups.iter().enumerate() {
                ...
                // XXX: This is different than SpringSharp (see gunString()) which seems to have
                // XXX: a bug when calculating this value for group 2
                let sb_super = sb.above < b.mount_num - b.groups[other].above;
                ...
                if sb.below > 0 {
                    println!("\t\t{} hull mount{} {}- Limited use in {}",
                        sb.below,
                        if sb.above > 1 { "s" } else if sb.distribution.super_aft() && main_gun { " aft" } else { "" },
        ...
        println!("\tWeight of broadside {:.0} lbs",
            self.wgt_broad());
        println!("");
        ...
        if self.armor.incline != 0.0 {
            println!("\tMain belt is inclined {:.2}° {}.",
                self.armor.incline,
                if self.armor.incline < 0.0 { "inward" } else { "outward" });
        }
        println!("\tMain Belt covers {:.0} % of normal length",
            self.armor.belt_coverage(self.hull.lwl())*100.0);
        if self.armor.belt_coverage(self.hull.lwl()) < self.hull_room() {
            println!("\tMain belt does not fully cover magazines and engineering spaces");
        }
        println!("");

        if self.armor.bulkhead.thick != 0.0 {
            println!("- Torpedo Bulkhead - Additional damage containing bulkheads:");
            println!("\t\t{:.2}\"\t{:.2} ft\t{:.2} ft",
                self.armor.bulkhead.thick,
                self.armor.bulkhead.len,
                self.armor.bulkhead.hgt);
            println!("\tBeam between torpedo bulkheads {:.2} ft",
                self.armor.beam_between);
        }
        println!("");

        if self.armor.bulge.thick != 0.0 {
            println!("- Hull void:");
            println!("\t\t{:.2}\"\t{:.2} ft\t{:.2}\"",
        ...
        println!("- Gun armour:\tFace (max)\tOther gunhouse (avg)\tBarbette/hoist (max)");
        for (i, b) in self.batteries.iter().enumerate() {
            if b.armor_face == 0.0 &&
               b.armor_back == 0.0 &&
            ...
            println!("\t{}:\t{}\t\t{}\t\t\t{}",
                ...
                if b.armor_face == 0.0 { "-".into() } else if b.armor_face >= 10.0 { format!("{:.1}\"", b.armor_face) } else { format!("{:.2}\"", b.armor_face) },
                if b.armor_back == 0.0 { "-".into() } else if b.armor_back >= 10.0 { format!("{:.1}\"", b.armor_back) } else { format!("{:.2}\"", b.armor_back) },
                if b.armor_barb == 0.0 { "-".into() } else if b.armor_barb >= 10.0 { format!("{:.1}\"", b.armor_barb) } else { format!("{:.2}\"", b.armor_barb) },
            );
        }
        println!("");
        ...
        println!("\tFor and Aft decks: {:.2}\"",
            self.armor.deck.md);
        // TODO: Change spelling to Quarterdeck
        println!("\tForecastle: {:.2}\"\tQuarter deck: {:.2}\"",
            self.armor.deck.fc,
            self.armor.deck.qd);
        println!("");
        // TODO: Remove stray space before comma
        println!("- Conning towers: Forward {:.2}\", Aft {:.2}\"",
        ...
        if self.engine.vmax != 0.0 {
            println!("\t{}, {},",
                self.engine.fuel,
                self.engine.boiler);
            println!("\t{}, {} shaft{}, {:.0} {} = {:.2} kts",
                self.engine.drive,
                self.engine.shafts,
                match self.engine.shafts { 1 => "", _ => "s", },
                (self.engine.hp_max(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()) as u64),
                self.engine.boiler.hp_type(),
                self.engine.vmax);
            println!("\tRange {}nm at {:.2} kts",
                self.engine.range,
                self.engine.vcruise);
            println!("\tBunker at max displacement = {:.0} tons",
                (self.engine.bunker_max(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()) as u64));
        } else {
            println!("\tImmobile floating battery");
        }
        println!("");
        ...
        println!("\t- Belts: {:.0} tons, {:.1} %",
            ...
            Ship::percent_calc(self.hull.d(), 
                ...
                self.armor.end.wgt(self.hull.lwl(), self.hull.cwp(), self.hull.b) +
                self.armor.upper.wgt(self.hull.lwl(), self.hull.cwp(), self.hull.b)));
        println!("\t- Torpedo bulkhead: {:.0} tons, {:.1} %",
        ...
        println!("\t- Conning Tower: {:.0} tons, {:.1} %",
        ...
        println!("\tMachinery: {:.0} tons, {:.1} %",
            self.wgt_engine(),
            Ship::percent_calc(self.hull.d(), self.wgt_engine()));
        println!("\tHull, fittings & equipment: {:.0} tons, {:.1} %",
            self.wgt_hull(),
            Ship::percent_calc(self.hull.d(), self.wgt_hull()));
        println!("\tFuel, ammunition & stores: {:.0} tons, {:.1} %",
            self.wgt_load(),
            Ship::percent_calc(self.hull.d(), self.wgt_load()));
        println!("\tMiscellaneous weights: {:.0} tons, {:.1} %",
            self.wgts.wgt(),
            Ship::percent_calc(self.hull.d(), self.wgts.wgt().into()));
        if self.wgts.vital > 0 { println!("\t- Hull below water: {:.0} tons", self.wgts.vital); }
        if self.wgts.void > 0 {
            println!("\t- {} void weights: {:.0} tons",
                if self.hull.bb > self.hull.b { "Bulge" } else { "Hull void" },
                self.wgts.void);
        }
        if self.wgts.hull > 0 { println!("\t- Hull above water: {:.0} tons", self.wgts.hull); }
        if self.wgts.on > 0 { println!("\t- On freeboard deck: {:.0} tons", self.wgts.on); }
        if self.wgts.above > 0 { println!("\t- Above deck: {:.0} tons", self.wgts.above); }

        println!("");

        println!("Overall survivability and seakeeping ability:"); // {{{3
        println!("\tSurvivability (Non-critical penetrating hits needed to sink ship):");
        println!("\t{} lbs = {:.1} x {:.1} \" shells or {:.1} torpedoes",
            self.flotation(),
            self.damage_shell_num(),
            self.damage_shell_size(),
            self.damage_torp_num());
        println!("\tStability (Unstable if below 1.00): {:.2}",
            self.stability_adj());
        println!("\tMetacentric height {:.1} ft",
            self.metacenter());
        println!("\tRoll period: {:.2} seconds",
            self.roll_period());
        println!("\tSteadiness\t- As gun platform (Average = 50 %): {:.0} %",
            self.steadiness());
        println!("\t\t- Recoil effect (Restricted arc if above 1.00): {:.2}",
            self.recoil());
        println!("\tSeaboat quality (Average = 1.00): {:.2}",
            self.seakeeping());
        println!("");
        ...
        if self.hull.is_wet_fwd() {
            println!("\tShip tends to be wet forward");
        }
        println!("");

        println!("Ship space, strength and comments:"); // {{{3
        println!("\tSpace\t- Hull below water (magazines/engines, low = better): {:.1} %",
            self.hull_room() * 100.0);
        println!("\t\t- Above water (accommodation/working, high = better): {:.1} %",
            self.deck_room() * 100.0);
        println!("\tWaterplane Area: {:.0} Square feet",
            (self.hull.wp() as u64));
        println!("\tDisplacement factor (Displacement / loading): {:.0} %",
            self.d_factor() * 100.0);
        println!("\tStructure weight / hull surface area: {:.0} lbs/sq ft",
            self.wgt_struct());
        println!("Hull strength (Relative):");
        println!("\t\t- Cross-sectional: {:.2}",
            self.str_cross());
        println!("\t\t- Longitudinal: {:.2}",
            self.str_long());
        println!("\t\t- Overall: {:.2}",
            self.str_comp());
        println!("\t{} machinery, storage, compartmentation space",
            self.hull_room_quality());
        println!("\t{} accommodation and workspace room",
        ...
        // TODO: Place these error labels in the status bar {{{3
        // if self.str_comp() < 0.5 {
            // println!("DESIGN FAILURE: Hull structure insufficient to carry required loads!");
        // } else if self.str_comp() < 0.995 {
            // if self.engine.vmax >= 24.0 && self.hull.d() < 4000.0 {
                // println!("Design well-balanced for a light, fast combatant");
            // } else {
                // println!("WARNING: Hull structure strained by open sea conditions! (Advisable for light, fast combatants only.)");
            // }
        // } else if self.str_comp() < 1.005 {
            // println!("Design well-balanced for a Battleship or Cruiser");
        // } else {
            // println!("Design under-gunned or otherwise under-performing relative to displacement");
        // }
    ...
    pub fn internals(&self) {
        eprintln!("Cs = {}", self.hull.cs());
        eprintln!("Cm = {}", Hull::cm(self.hull.cb()));
        eprintln!("Cp = {}", Hull::cp(self.hull.cb()));
        eprintln!("Cwp = {}", self.hull.cwp());
        eprintln!("WP = {}", self.hull.wp());
        eprintln!("WS = {}", self.hull.ws());
        eprintln!("Ts = {}", self.hull.ts());
        eprintln!("");
        eprintln!("Stem length = {}", self.hull.stem_len());
        if let BowType::Ram(len) = self.hull.bow_type { eprintln!("Ram length = {}", len); }
        eprintln!("Freeboard dist = {}", self.hull.freeboard_dist());
        eprintln!("Leff = {}", self.hull.leff());
        eprintln!("");
        eprintln!("Rf max = {}", self.engine.rf_max(self.hull.ws()));
        eprintln!("Rf cruise = {}", self.engine.rf_cruise(self.hull.ws()));
        eprintln!("Rw max = {}", self.engine.rw_max(self.hull.d(), self.hull.lwl(), self.hull.cs()));
        eprintln!("Rw cruise = {}", self.engine.rw_cruise(self.hull.d(), self.hull.lwl(), self.hull.cs()));
        eprintln!("Pw max = {}", self.engine.pw_max(self.hull.d(), self.hull.lwl(), self.hull.cs(), self.hull.ws()));
        eprintln!("Pw cruise = {}", self.engine.pw_cruise(self.hull.d(), self.hull.lwl(), self.hull.cs(), self.hull.ws()));
        eprintln!("");
        eprintln!("hp max = {}", self.engine.hp_max(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()));
        eprintln!("hp cruise = {}", self.engine.hp_cruise(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()));
        eprintln!("");

        eprintln!("wgt_load = {}", self.wgt_load());
        eprintln!("wgt_hull = {}", self.wgt_hull());
        eprintln!("wgt_hull_plus = {}", self.wgt_hull_plus());
        eprintln!("wgt_misc = {}", self.wgts.wgt());
        eprintln!("wgt_armor = {}", self.wgt_armor());
        eprintln!("");

        eprintln!("main belt = {}", self.armor.main.wgt(self.hull.d(), self.hull.cwp(), self.hull.b));
        eprintln!("upper belt = {}", self.armor.upper.wgt(self.hull.d(), self.hull.cwp(), self.hull.b));
        eprintln!("end belt = {}", self.armor.end.wgt(self.hull.d(), self.hull.cwp(), self.hull.b));
        eprintln!("deck = {}", self.armor.deck.wgt(self.hull.clone(), self.wgt_mag(), 0.0));
        eprintln!("");

        eprintln!("wgt_guns = {}", self.wgt_guns());
        eprintln!("wgt_gun_mounts = {}", self.wgt_gun_mounts());
        eprintln!("wgt_mag = {}", self.wgt_mag());
        eprintln!("wgt_gun_armor = {}", self.wgt_gun_armor());
        eprintln!("wgt_borne = {}", self.wgt_borne());
        eprintln!("super_factor = {}", self.gun_super_factor());
        eprintln!("gun_wtf = {}", self.gun_wtf());
        eprintln!("");

        eprintln!("wgt_engine = {}", self.wgt_engine());
        eprintln!("d_engine = {}", self.engine.d_engine(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()));
        eprintln!("d_factor = {}", self.d_factor());
        eprintln!("");

        eprintln!("stability = {}", self.stability());
        eprintln!("seaboat = {}", self.seaboat());
        eprintln!("");

        eprintln!("{:?}", self.engine.fuel);
        eprintln!("{:?}", self.engine.boiler);
        eprintln!("{:?}", self.engine.drive);
        eprintln!("num_engines = {}", self.engine.num_engines());

        eprintln!("");

        eprintln!("gun_concentration = {}", self.gun_concentration());
        eprintln!("str_cross = {}", self.str_cross());
    }
}
...
impl Ship {
    ...
    pub fn flotation(&self) -> f64 {
        ...
        let b = (a * self.hull.cwp() / Hull::FT3_PER_TON_SEA + self.hull.d()) / 2.0;
    ...
    pub fn str_cross(&self) -> f64 {
        ...
        str_cross
         ...
         // (
         //     self.wgt_struct() /
         //     (self.hull.bb * (self.hull.t + self.hull.freeboard_dist())).sqrt() /
         //     (
         //         (
         //             self.hull.d() + (
         //                 (self.wgt_broad() + self.wgt_borne() + self.wgt_gun_armor() + self.armor.ct_fwd.wgt(self.hull.d() + self.armor.ct_aft.wgt(self.hull.d()))) *
         //                 ((1.0 + self.gun_concentration()) * self.gun_super_factor()) +
         //                 f64::max(self.engine.hp_max(self.hull.d(), self.hull.lwl(), self.hull.leff(), self.hull.cs(), self.hull.ws()), 0.0) / 100.0
         //                )
         //          ) / self.hull.d()
         //      ) * 0.6)

         // * if self.year < 1900 { (1 - (1900 - self.year) / 100) as f64 } else { 1.0 }
    }
