// sship_enum {{{1
/// Generate format-layer support for an enum stored by `.sship` index.
///
/// Expands to `ALL`, `From<&str>`, `From<String>` and `fmt::Display`. The
/// variants must be listed in SpringSharp (.sship) index order, which is
/// not always declaration order. Display strings are report prose, not
/// GUI labels. Parsing accepts any non-negative decimal integer; unknown
/// or out-of-range values yield the default variant.
///
macro_rules! sship_enum {
    ($name:ident { $( $variant:ident $( ( $init:expr ) )? => $display:expr ),+ $(,)? }) => {
        impl $name {
...
// gui_enum {{{1
/// Generate view-layer support for an enum shown in dropdowns.
///
/// Expands to `label()`, `index()` and `from_index()`, delegating to the
/// `ALL` slice generated by `sship_enum!`. Labels are keyed by variant
/// name and may be listed in any order.
///
macro_rules! gui_enum {
    ($name:ident { $( $variant:ident => $label:expr ),+ $(,)? }) => {
        impl $name {
            /// Label for display in the GUI.
            ///
            ...
            /// Index into ALL, matching the .sship encoding.
            ///
            ...
            /// The variant for an index into ALL.
            ///
